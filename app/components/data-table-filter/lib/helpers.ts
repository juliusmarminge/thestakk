...
export function createDateFilterValue(values: [Date, Date] | [Date] | [] | undefined) {
  ...
  if (values.length === 2)
    return createDateRange([
      DateTime.unsafeFromDate(values[0]),
      DateTime.unsafeFromDate(values[1]),
    ]);
  throw new Error("Cannot create date filter value from more than 2 values");
}

export function createDateRange(values: [DateTime.Utc, DateTime.Utc]) {
  const [a, b] = values;
  const [min, max] = DateTime.lessThan(a, b) ? [a, b] : [b, a];
